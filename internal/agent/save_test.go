...
func TestSaveRecoveryBranchInheritsValidProjection(t *testing.T) {
	...
	recoveredMsgs, recoveredVersion := recovered.snapshotMessagesVersion()
	if !projectionContentValid(got, recoveredMsgs, recoveredVersion) {
		t.Fatalf("inherited projection does not validate against the recovery transcript")
	}
}
